...
let content = React.functionComponent(fun (input: {| state: State; dispatch: Msg -> unit |}) ->
    ...
    | [ Urls.SweetAlert; Urls.Overview; ] -> lazyView MarkdownLoader.load [ "SweetAlert"; "README.md" ]
    | [ Urls.SweetAlert; Urls.Installation ] -> lazyView MarkdownLoader.load [ "SweetAlert"; "Installation.md" ]
    | [ Urls.SweetAlert; Urls.ReleaseNotes ] -> lazyView MarkdownLoader.load [ "SweetAlert"; "RELEASE_NOTES.md" ]
    | [ Urls.SweetAlert; Urls.Contributing ] -> lazyView MarkdownLoader.load [ contributing ]
    | PathPrefix [ Urls.SweetAlert; Urls.Examples ] (Some res) ->
        match res with
        | PathPrefix [ Urls.Elmish ] (Some innerRes) -> elmishExamples innerRes
        | PathPrefix [ Urls.Simple ] (Some innerRes) -> simpleExamples innerRes
        | [ Urls.React ] -> [ "React.md" ]
        ...
        | [ Urls.DynamicQueue ] -> [ "DynamicQueue.md" ]
        | _ -> []
        |> fun path -> [ Urls.SweetAlert; Urls.Examples ] @ path |> lazyView MarkdownLoader.load
    | _ -> lazyView MarkdownLoader.load [ "SweetAlert"; "README.md" ])
